...
public class ChatbotGeneric {

    static String[] DefaultResponses = {
            "Let's change the subject",
            "I'm not sure I know what you're talking about",
            "Wooooah, that is not cool!",
    ...
    public ChatbotGeneric() {
    }

    public String ask(String question) {
        String trimmed = question.replace(",", "").replace(".", "").replace("?", "");
        String capitalized = trimmed.toUpperCase();
        for (String[][] queryResponse: KnowledgeBase) {
            String queryStart = queryResponse[0][0];
            if (capitalized.startsWith(queryStart)) {
                String[] responses = queryResponse[1];
                int rnd = new Random().nextInt(responses.length);
                return responses[rnd];
            }
        }
        int rnd = new Random().nextInt(DefaultResponses.length);
